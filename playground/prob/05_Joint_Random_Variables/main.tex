...
% If $X$ and $Y$ are instead defined on different experiments with sample spaces $S_{1}$ and $S_{2}$, we may set $S=S_{1} \times S_{2}$, or a subset thereof, consisting of pairs $s \equiv(s_{1}, s_{2}), s_{1} \in S_{1}, s_{2} \in S_{2}$.
...
% $$
% \mathrm{P}_{Z}(X \leq x, Y \leq y)=\mathrm{P}_{Z}((-\infty, x],(-\infty, y])=\mathrm{P}(S_{x y})
% $$
...
% $x_{1}<x_{2} \Rightarrow F(x_{1}, y_{1}) \leq F(x_{2}, y_{1})$ and\\
% $y_{1}<y_{2} \Rightarrow F(x_{1}, y_{1}) \leq F(x_{1}, y_{2})$;\\
% (3) $F(x,-\infty)=F(-\infty, y)=0$ and $F(\infty, \infty)=1 \quad \forall x, y \in \mathbb{R}$.

% \section*{Interval Probabilities}
% Suppose we are interested in whether the random variable pair $Z=(X, Y)$ lie in the interval cross product $(x_{1}, x_{2}] \times(y_{1}, y_{2}]$; that is, if $x_{1}<X \leq x_{2}$ and $y_{1}<Y \leq y_{2}$.\\
% \includegraphics[max width=\textwidth, center]{2025_05_11_84d245f69223a25e0522g-07}

% \begin{itemize}
%   \item $\mathrm{P}_{Z}(x_{1}<X \leq x_{2}, Y \leq y)=F(x_{2}, y)-F(x_{1}, y)$
%   \item Hence:
% \end{itemize}
...
$$
p(n_{1}, \ldots, n_{r})=\mathrm{P}_{Z}(X_{1}=n_{1}, \ldots, X_{r}=n_{r})=\frac{n!}{n_{1}!n_{2}!\cdots n_{r}!} q_{1}^{n_{1}} q_{2}^{n_{2}} \cdots q_{r}^{n_{r}}
$$

This is due to independence, since a sequence has probability

$$
q_{1}^{n_{1}} q_{2}^{n_{2}} \cdots q_{r}^{n_{r}} \quad(n_{1}+n_{2}+\ldots+n_{r}=n)
$$

and the number of sequences that yield $(n_{1}, \ldots, n_{r})$ is
...
% $$
% \mathrm{E}(g_{1}(X)+g_{2}(Y))=E_{X}(g_{1}(X))+E_{Y}(g_{2}(Y)) .
% $$
...
% $$
% \mathrm{E}(g_{1}(X) g_{2}(Y))=\mathrm{E}_{X}(g_{1}(X)) \mathrm{E}_{Y}(g_{2}(Y)) .
% $$
...
% \section*{Covariance}
% \begin{itemize}
%   \item For a single r.v. $X$ we considered the expectation of $g(X)=(X-\mu_{X})(X-\mu_{X})$, i.e., the variance and denoted $\sigma_{X}^{2}$.
%   \item The bivariate extension of this is the expectation of $g(X, Y)=(X-\mu_{X})(Y-\mu_{Y})$.
%   \item We define the covariance of $X$ and $Y$ by
% \end{itemize}

% $$
% \begin{aligned}
% \sigma_{X Y}=\operatorname{Cov}(X, Y) & =\mathrm{E}[(X-\mu_{X})(Y-\mu_{Y})] \\
...
A random vector $X=(X_{1}, \ldots, X_{n})$ with means $\mu=(\mu_{1}, \ldots, \mu_{n})$ that has joint pdf

$$
f_{X}=\frac{1}{\sqrt{(2 \pi)^{n} \operatorname{det} \Sigma}} \exp (-\frac{1}{2}(x-\mu)^{T} \Sigma^{-1}(x-\mu))
$$

is said to have a multivariate Normal distribution, where\\
$\mu=(\mu_{1}, \ldots, \mu_{n})$ is the vector of means of $X_{1}, \ldots, X_{n}$\\
and the covariance matrix $\Sigma=[\operatorname{Cov}(X_{i}, X_{j}) ; 1 \leq i, j \leq n]$, which must be positive definite for a pdf to exist.

Note that the r.vs. $X_{1}, \ldots, X_{n}$ need not be independent.

\section*{Example: Independent normal r.vs.}
$$
\mu=(1,1), \operatorname{Var}(X_{1})=\operatorname{Var}(X_{2})=1, \operatorname{Cov}(X_{1}, X_{2})=\operatorname{Cov}(X_{2}, X_{1})=0
$$
...
$$
\mu=(1,1), \operatorname{Var}(X_{1})=\operatorname{Var}(X_{2})=1, \operatorname{Cov}(X_{1}, X_{2})=\operatorname{Cov}(X_{2}, X_{1})=0.9
$$
...
& =\int_{y=-\infty}^{\infty} F_{X}(y) f_{Y}(y) \mathrm{d} y=\int_{0}^{\infty}(1-e^{-\lambda y}) \mu e^{-\mu y} \mathrm{~d} y \\
...
& =\int_{0}^{\infty}(1-e^{-\lambda y}) \mu e^{-\mu y} \mathrm{~d} y \text { (by independence) } \\
& =1-\frac{\mu}{\lambda+\mu}=\frac{\lambda}{\lambda+\mu} .
\end{aligned}
$$
...
% $$
% \mathrm{E}_{Y}(Y)=\mathrm{E}_{X}(\mathrm{E}_{Y \mid X}(Y \mid X))
% $$
...
% of i.i.d. r.vs. $X_{0}, X_{1}, \ldots$, taking values in $J=\operatorname{supp}(X_{i})=\{0,1\}$.\\
...
% The goal is to calculate $P(X_{n}=j)$, i.e., the probability that at time $n$ the system reaches state $j$.

% \section*{Homogeneous DTMCs}
% DTMCs assume that the following Markov property holds:

% $$
% P(X_{n+1}=j_{n+1} \mid X_{n}=j_{n}, \ldots, X_{1}=j_{1}, X_{0}=j_{0})=P(X_{n+1}=j_{n+1} \mid X_{n}=j_{n})
% $$
...
% \begin{itemize}
%   \item An initial probability vector $\pi_{0}=[\pi_{0 i}]$, i.e., $P(X_{0}=i)=\pi_{0 i}$.
%   \item A transition probability matrix $R=[r_{i j}]$, where
% \end{itemize}

% $$
% r_{i j}=P(X_{n+1}=j \mid X_{n}=i)
% $$
...
% $$
% \begin{aligned}
% P(X_{n+2}=j \mid X_{n}=i) & =\sum_{k \in J} P(X_{n+2}=j \mid X_{n+1}=k) P(X_{n+1}=k \mid X_{n}=i) \\
% & =\sum_{k \in J} r_{i k} r_{k j}=(R^{2})_{i j}
% \end{aligned}
% $$
...
% $$
% \begin{aligned}
% P(X_{n}=j) & =\sum_{i \in J} P(X_{n}=j \mid X_{0}=i) P(X_{0}=i) \\
% & =\sum_{i \in J} P(X_{0}=i) P(X_{n}=j \mid X_{0}=i) \\
% & =\sum_{i \in J} \pi_{0 i}(R^{n})_{i j} \\
% & =(\pi_{0} R^{n})_{j}
% \end{aligned}
% $$

% i.e. the $j$-th element of vector $\pi_{0} R^{n}$ gives us $P(X_{n}=j)$.
...
$$
\pi_{0} R^{2}=[\begin{array}{ll}
0 & 1
\end{array}][\begin{array}{ll}
0.688 & 0.312 \\
0.625 & 0.375
\end{array}]=[\underbrace{0.625}_{P(X_{2}=1)} 0.375]
$$

\begin{itemize}
  \item What is the long-term probability of hot and cold days?\\
$\lim _{n \rightarrow+\infty} \pi_{0} R^{n}=[\begin{array}{ll}\pi_{01} & \pi_{02}\end{array}][\begin{array}{ll}0.667 & 0.333 \\ 0.667 & 0.333\end{array}]=[2 / 3,1 / 3]=\pi_{\infty}$\\
...
% $$
% P(X_{n}=j)=\pi_{\infty, j}^{*}, \quad \forall n \geq 0, \forall j \in J .
% $$
...
% $$
% R_{1}=(\begin{array}{ccc}
% \frac{1}{3} & \frac{1}{3} & \frac{1}{3} \\
% \frac{1}{4} & \frac{3}{4} & 0 \\
% 0 & 0 & 1
% \end{array}) \quad R_{2}=(\begin{array}{ccc}
% 0 & 1 & 0 \\
% 0 & 0 & 1 \\
% 1 & 0 & 0
% \end{array}) \quad R_{3}=(\begin{array}{ccc}
% \frac{1}{3} & \frac{1}{3} & \frac{1}{3} \\
% \frac{1}{2} & 0 & \frac{1}{2} \\
...
$\pi_{\infty}[\begin{array}{cc}3 / 4 & 1 / 4 \\ 1 / 2 & 1 / 2\end{array}]=\pi_{\infty} \Rightarrow \pi_{\infty}[\begin{array}{cc}-1 / 4 & 1 / 4 \\ 1 / 2 & -1 / 2\end{array}]=(0,0)$ (singular)\\
Now, replacing an equation with $\sum_{j} \pi_{\infty, j}=1$ we get instead

$$
\pi_{\infty}[\begin{array}{cc}
-1 / 4 & 1 \\
1 / 2 & 1
\end{array}]=(0,1) \Rightarrow \pi_{\infty}=(2 / 3,1 / 3)
$$
