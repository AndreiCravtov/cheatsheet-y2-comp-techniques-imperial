...
For \iMbox{A \in \mathbb{R}^{m \times n}}, \iMbox{\ds A_{ij}} is the \iMbox{i}-th \textbf{ROW} then \iMbox{j}-th \textbf{COLUMN} \\
\iMbox{\ds (A^T)_{ij} = A_{ji}}
\iMbox{\ds (AB)_{ij} = A_{i\ast} \cdot B_{\ast j} = \sum_{k} A_{ik}B_{kj}}
\iMbox{\ds (Ax)_{i} = A_{i\ast} \cdot x = \sum_{j} A_{ij} x_{j}}
\iMbox{\ds x^Ty = y^Tx = x \cdot y = \sum_{i} x_{i} y_{i}}
\iMbox{\ds x^TAx = \sum_{i} \sum_{j} A_{ij}x_{i}x_{j}}
...
\begin{itemize}
  \item
    ...
    \iMbox{\lambda A = \lambda [A_{1}|A_{2}|\dots] = [\lambda A_{1}|\lambda A_{2}|\dots]}
  \item
    \textbf{row-blocks} =\textgreater{}
    \iMbox{\lambda A = \lambda [A_{1};A_{2};\dots] = [\lambda A_{1};\lambda A_{2};\dots]}
\end{itemize}

Consider \iMbox{A,B \in \mathbb{R}^{m \times n}} \textbf{partitioned  column/row-wise} in the same way => matrix-addition distributes over:
\begin{itemize}
  \item
    \textbf{column-blocks} =>
    \iMbox{A+B = [A_{1}|A_{2}|\dots] + [B_{1}|B_{2}|\dots] = [A_{1}+B_{1}|A_{2}+B_{2}|\dots]}
  \item
    \textbf{row-blocks} =>
    \iMbox{A+B = [A_{1};A_{2};\dots] + [B_{1};B_{2};\dots] = [A_{1}+B_{1};A_{2}+B_{2};\dots]}
  \end{itemize}

Consider \iMbox{A = [A_{1};A_{2},\dots]\in \mathbb{R}^{m \times k}, B = [B_{1}|B_{2}|\dots] \in \mathbb{R}^{k \times n}}
  => matrix-multiplication distributes over:
\begin{itemize}
  \item
    \textbf{column-blocks} =\textgreater{}
    \iMbox{\ds AB = A[B_{1}|B_{2}|\dots] = [AB_{1}|AB_{2}|\dots]}
  \item
    \textbf{row-blocks} =\textgreater{}
    \iMbox{AB = [A_{1};A_{2};\dots]B = [A_{1}B;A_{2}B;\dots]}
\end{itemize}

Consider
  \iMbox{\ds A = [A_{1}|\dots|A_{p}]\in \mathbb{R}^{m \times k}, 
  \ds B = [B_{1};\dots; B_{p}] \in \mathbb{R}^{k \times n}}
  => \textbf{\emph{outer-product sum}} equivalence:
\begin{itemize}
  \item
    If partition-sizes match =\textgreater{}
    \iMbox{\ds AB = \sum_{i=1}^{p} A_{i}B_{i}}
  \item
    e.g.~for \iMbox{\ds A = [\mathbf{a}_{1}|\dots|\mathbf{a}_{n}]},
    \iMbox{B=[\mathbf{b}_{1}; \dots; \mathbf{b}_{n}]} =\textgreater{}
...
\item
  ...
  \iMbox{\ds\mathrm{proj}_{u}(v) = \frac{u \cdot v}{u \cdot u} u}

  \begin{itemize}
  
  \item
    \iMbox{\ds \hat{u} = \frac{u}{\lVert u \rVert}} so \iMbox{\hat{u}} a
    \textbf{unit vector on the line} containing \iMbox{\hat{u}}
  \item
    So we get
    \iMbox{\ds \mathrm{proj}_{u}(v) = \hat{u} \hat{u}^{T}v = \frac{1}{\lVert u \rVert\lVert u \rVert}u u^{T} v =\frac{1}{\lVert u \rVert^{2}} u (u \cdot v) = \frac{u \cdot v}{u \cdot u} u}
  \item
...
\item
  ...
  \item
    Let
    \iMbox{\ds \mathbf{U} = [ \mathbf{u}_{1}|\dots|\mathbf{u}_{k} ] \in \mathbb{R}^{m \times k}}
    be the matrix of columns \iMbox{\mathbf{u}_{1},\dots,\mathbf{u}_{k}}
  \item
    Then \textbf{orthogonal projection onto the subspace} \iMbox{U} is
    \iMbox{\ds \pi_{U} = \mathbf{U} \mathbf{U}^{T}}
  \item
    Can be rewritten as
  ...
  \item
    ...
    \iMbox{(\mathbf{U}^{T}\mathbf{U})^{-1}} is added =\textgreater{}
...
\item
  ...
  \item
    ...
    \item
      ...
      \iMbox{\mathbf{q}_{1},\dots,\mathbf{q}_{j}}
    \item
      \iMbox{\ds \mathrm{P}_{j} = Q_{j}Q_{j}^{T}} is {[}{[}\#Projection
    ...
    \end{itemize}
  \item
    Assume \iMbox{\mathbf{a}_{j+1} \not\in U_{j}} =\textgreater{} unique
    decomposition
    \iMbox{\ds \mathbf{a}_{j+1} = \mathbf{v}_{j+1} + \mathbf{u}_{j+1}}

    \begin{itemize}
    
    \item
      \iMbox{\ds\mathbf{v}_{j+1} = \mathrm{P}_{j}\left(\mathbf{a}_{j+1}\right) \in U_{j}}
      =\textgreater{} discard it!!
    \item
      \iMbox{\ds\mathbf{u}_{j+1} = \mathrm{P}_{\perp j}\left(\mathbf{a}_{j+1}\right) \in \left(U_{j}\right)^{\perp}}
  ...
  \item
    ...
    \iMbox{\ds U_{j+1}} =\textgreater{} start next iteration
...
\item
  Let \iMbox{\mathbf{v}_{1},\dots,\mathbf{v}_{k} \in \mathbb{R}^m} be
  linearly independent
\item
...
\item
  ...
  \item
    ...
    and \iMbox{C}
  \item
    \iMbox{\ds f(\mathbf{b}_{j}) = \sum_{i=1}^{m} A_{ij} \mathbf{c}_{i}}
    --\textgreater{} each \iMbox{\mathbf{b}_{j}} basis gets mapped to a
    linear combination of \iMbox{\ds \sum_{i} a_{i}\mathbf{c}_{i}} bases
  \item
...
\item
  ...
  \item
    \iMbox{\mathbf{I}_{BE} = (\mathbf{I}_{EB})^{-1}}, so =\textgreater{}
...
\item
  ...
  \item
    If \iMbox{a_{i} = x \cdot \mathbf{b}_{i}} then
    \iMbox{\ds x = \sum_{i} a_{i} \mathbf{b}_{i}}, we call \iMbox{a} the
...
\item
  ...
  the subspace
  \iMbox{\ds U^{\perp} = \left\{ \ x \in \mathbb{R} ^{n} \ \middle| \ \forall y\in \mathbb{R}^{n}:x \perp y \ \right\} = \left\{ \ x \in \mathbb{R} ^{n} \ \middle| \ \forall y\in \mathbb{R}^{n}: \lVert x \rVert \leq \lVert x +y \rVert \ \right\}}

  \begin{itemize}
  
  \item
    \iMbox{\mathbb{R}^{n} = U \oplus U^{\perp}} and
    \iMbox{(U^{\perp})^{\perp} = U} \emph{(because finite dimensional)}
  \item
    \iMbox{U \perp V \iff U^{\perp} = V} and vice-versa \emph{(because
    finite dimensional)}
  \item
...
\item
  \iMbox{a^2+b^2=c^2} (\textbf{Pythagorean theorem})
\item
  \iMbox{c=\sqrt{a^2+b^2-2 a b \cdot \cos \gamma}} (\textbf{law of
  cosines})
\item
  \iMbox{\ds\frac{\sin A}{a}=\frac{\sin B}{b}=\frac{\sin C}{c}}
  (\textbf{law of sines})
\item
...
\item
  \iMbox{\ell_{p}} norms:
  \iMbox{\ds \|\boldsymbol{x}\|_p=\left(\sum_{i=1}^n\left|x_i\right|^p\right)^{1 / p}}

  \begin{itemize}
  
  \item
    \iMbox{p=1}:
    \iMbox{\ds\|\boldsymbol{x}\|_1=\sum_{i=1}^n\left|x_i\right|}
  \item
    \iMbox{p=2}:
    \iMbox{\ds \|\boldsymbol{x}\|_2=\sqrt{\sum_{i=1}^n x_i^2} = \sqrt{ \boldsymbol{x} \cdot \boldsymbol{x} }}
  \item
    \iMbox{p=\infty}:
    \iMbox{\ds \|\boldsymbol{x}\|_{\infty} = \lim _{p \rightarrow \infty}\|\boldsymbol{x}\|_p =\max _{1 \leq i \leq n}\left|x_i\right|}
  \end{itemize}
\item
  Any two norms in \iMbox{\mathbb{R}^n} are equivalent, meaning there
  exist \iMbox{r>0,s>0} such that:
  \iMbox{\ds\forall \boldsymbol{x} \in \mathbb{R}^n, r\|\boldsymbol{x}\|_a \leq\|\boldsymbol{x}\|_b \leq s\|\boldsymbol{x}\|_a}
  ...
  \item
    ...
    =\textgreater{}
    \iMbox{\begin{aligned} & \|\boldsymbol{x}\|_{\infty} \leq\|\boldsymbol{x}\|_2 \leq\|\boldsymbol{x}\|_1 \\ & \|\boldsymbol{x}\|_2 \leq \sqrt{n}\|\boldsymbol{x}\|_{\infty} \\ & \|\boldsymbol{x}\|_1 \leq \sqrt{n}\|\boldsymbol{x}\|_2\end{aligned}}
  \end{itemize}
\item
...
\item
  ...
  \iMbox{\boldsymbol{A} \in \mathbb{R}^{m \times n}}:

  \begin{itemize}
  
  \item
    \iMbox{\ds\|\boldsymbol{A}\|_1=\max _j\left\|\boldsymbol{A}_{\ast j}\right\|_1}
  \item
    \iMbox{\ds \|\boldsymbol{A}\|_2=\sigma_1(\boldsymbol{A})}
    i.e.~largest singular value of \iMbox{A} \emph{(square-root of
    {[}{[}tutorial 3\#Singular Value Decomposition (SVD) \& Singular
    Values\textbar largest eigenvalue{]}{]} of \iMbox{A^TA} or
    \iMbox{AA^T})}
  \item
    \iMbox{\ds\|\boldsymbol{A}\|_{\infty} =\max_i \left\|\boldsymbol{A}_{i\ast}\right\|_1},
    note that
    \iMbox{\ds \|\boldsymbol{A}\|_1=\left\|\boldsymbol{A}^T\right\|_{\infty}}
  \item
    \textbf{Frobenius norm}:
    \iMbox{\ds \|\boldsymbol{A}\|_F=\sqrt{\sum_{i=1}^m \sum_{j=1}^n\left|\boldsymbol{A}_{i j}\right|^2}}
  \end{itemize}
\item
  ...
  \item
    for all \iMbox{\boldsymbol{A} \in \mathbb{R}^{m \times n}} and
    \iMbox{\boldsymbol{x} \in \mathbb{R}^n} =\textgreater{}
    \iMbox{\|\boldsymbol{A} \boldsymbol{x}\|_b \leq\|\boldsymbol{A}\|\|\boldsymbol{x}\|_a}
  \item
    If \iMbox{a=b,\|\cdot\|} is \textbf{compatible} with
    \iMbox{\|\cdot\|_a}
  \item
    Frobenius norm is \textbf{consistent} with \iMbox{\ell_{2}} norm
    =\textgreater{} \iMbox{\ds \|A v\|_2 \leq\|A\|_F\|v\|_2}.
  \end{itemize}
\item
  ...
  \iMbox{\|\boldsymbol{A}\|=\max \left\{\|\boldsymbol{A} \boldsymbol{x}\|: \boldsymbol{x} \in \mathbb{R}^n,\|\boldsymbol{x}\|=1\right\}}

  \begin{itemize}
  
  \item
    Alternative expressions:
    \iMbox{\begin{aligned}\|\boldsymbol{A}\| & =\max \left\{\|\boldsymbol{A} \boldsymbol{x}\|: \boldsymbol{x} \in \mathbb{R}^n,\|\boldsymbol{x}\|=1\right\} \\ & =\max \left\{\frac{\|\boldsymbol{A} \boldsymbol{x}\|}{\|\boldsymbol{x}\|}: \boldsymbol{x} \in \mathbb{R}^n, \boldsymbol{x} \neq 0\right\} \\ & =\max \left\{\|\boldsymbol{A} \boldsymbol{x}\|: \boldsymbol{x} \in \mathbb{R}^n,\|\boldsymbol{x}\| \leq 1\right\}\end{aligned}}
  \end{itemize}
\item
...
say the index \iMbox{M} is special (e.g.~maybe
\iMbox{\ds\left\|\boldsymbol{A}_{M \ast}\right\|_1 = \max_i \left\|\boldsymbol{A}_{i \ast}\right\|_1})
- Then you could pick a vector \iMbox{\ds x_{M}} based \textbf{on a
function of \iMbox{M}} -
e.g.~\iMbox{\ds (x_{M})_{j} = \mathrm{sgn}(\boldsymbol{A}_{M j})} can
help prove
\iMbox{\ds x_{M} \cdot \boldsymbol{A}_{M \ast} = \lVert \boldsymbol{A}_{M \ast} \rVert_{1}}
-
e.g.~\iMbox{\ds (x_{M})_{j} = \begin{cases} 1 & j=M, \\ 0 & j \neq M \end{cases}}
...
\item
  \iMbox{A} is symmetric \textbf{iff} \iMbox{A = A^{T}}

  \begin{itemize}
  
  \item
    \iMbox{A} is Hermitian, iff \iMbox{A = A^{\dagger}}, i.e.~its equal
  ...
  \item
    ...
    \item
      and
      \iMbox{\text{geometric multiplicity of} \ \lambda_{i} = \text{geometric multiplicity of} \ \lambda_{i}}
    \item
      and eigenvectors \iMbox{\mathbf{x}_{1},\mathbf{x}_{2}} associated
...
\item
  ...
  \item
    \textbf{Triangular matrices} =\textgreater{}
    \iMbox{\ds \left|A\right| = \prod_{i}a_{ii}}, i.e.~the product of
    diagonal elements
  \end{itemize}
\item
  ...
  \item
    \emph{Sometimes refers to rectangular matrices}, but most often
  ...
  \item
    ...
    \iMbox{Ax = \mathrm{diag}_{m \times n}(a_{1},\dots,a_{p})[x_{1} \dots x_{n}]^{T} = [a_{1}x_{1} \dots a_{p}x_{p} \ \ 0 \dots 0]^{T} \in \mathbb{R}^{m}}
    \emph{(if \iMbox{p=m} those tail-zeros don't exist)}
  \item
    Consider \iMbox{\mathrm{diag}_{m \times n}(\mathbf{b})}, then
  ...
  \item
    ...
    then
    \iMbox{\mathrm{diag}_{m \times n}(a_{1},\dots,a_{p})\mathrm{diag}_{n \times k}(c_{1},\dots,c_{q}) = \mathrm{diag}_{m \times k}(a_{1}c_{1},\dots,a_{r}c_{r}, 0,\dots,0) = \mathrm{diag}(\mathbf{s})}
    ...
    \end{itemize}
  \item
    \textbf{Inverse of square-diagonals} =\textgreater{}
    \iMbox{\mathrm{diag}(a_{1},\dots,a_{n})^{-1} = \mathrm{diag}(a_{1}^{-1},\dots,a_{n}^{-1})},
    i.e.~diagonals \textbf{cannot be zero} \emph{(division by zero
    undefined)}
  \item
    \textbf{Determinant of square-diagonals} =\textgreater{}
    \iMbox{\ds \left|\mathrm{diag}(a_{1},\dots,a_{n})\right| = \prod_{i}a_{i}}
...
\item
  Two matrices
  \iMbox{\boldsymbol{A}, \widetilde{\boldsymbol{A}} \in \mathbb{R}^{m \times n}}
  are \textbf{equivalent} if there exist two invertible matrices
  \iMbox{\boldsymbol{P} \in} \iMbox{\mathbb{R}^{m \times m}} and
  \iMbox{\boldsymbol{Q} \in \mathbb{R}^{n \times n}} such that
  \iMbox{\boldsymbol{A}=\boldsymbol{P} \widetilde{\boldsymbol{A}} \boldsymbol{Q}^{-1}}
\item
  Two matrices
  \iMbox{\boldsymbol{A}, \widetilde{\boldsymbol{A}} \in \mathbb{R}^{n \times n}}
  are \textbf{similar} if there exists an invertible matrix
  \iMbox{\boldsymbol{P} \in} \iMbox{\mathbb{R}^{n \times n}} such that
  \iMbox{\boldsymbol{A}=\boldsymbol{P} \widetilde{\boldsymbol{A}} \boldsymbol{P}^{-1}}

  \begin{itemize}
  
  \item
    \textbf{Similar} matrices are equivalent, with
    \iMbox{\boldsymbol{Q} = \boldsymbol{P}}
  \end{itemize}
\item
...
\item
  ...
  \iMbox{\ds A = [a_{1}|\dots| a_{j} | \dots|a_{n}] = [a_{1}|\dots| \lambda x_{i} + \mu y_{i}| \dots|a_{n}]}
  then
  \iMbox{\ds \det(A) = \lambda \det\left( [a_{1}|\dots| x_{i} | \dots|a_{n}] \right) + \mu \det\left( [a_{1}|\dots| y_{i} | \dots|a_{n}] \right)}

  \begin{itemize}
  
  \item
    And the exact same linearity property for \textbf{rows}
    \iMbox{A=[a_{1};\dots;a_{n}]}
  \item
...
\item
  ...
  \item
    ...
    \iMbox{\mathrm{rk}(A) < n \iff |A| = 0} \textless=\textgreater{}
...
\item
  ...
  \item
    ...
    e.g.~\iMbox{\ds \det\left( [a_{1}|\dots| a_{i} | \dots | a_{j} | \dots |a_{n}] \right) = -\det\left( [a_{1}|\dots| a_{j} | \dots | a_{i} | \dots |a_{n}] \right)}
  \item
...
\item
  ...
  \item
    \iMbox{\det \begin{pmatrix}A & B \\ 0 & D\end{pmatrix} = \det(A)\det(B) = \det \begin{pmatrix}A & 0 \\ C & D\end{pmatrix}}
  \item
    \iMbox{\det \begin{pmatrix}A & B \\ C & D\end{pmatrix} = \det(A)\det(D - CA^{-1}B) = \det(D)\det(A - BD^{-1}C)}
...
\item
  ...
  \item
    ...
    \iMbox{\ds A \rightsquigarrow_{\mathrm{EROs}} A'} =\textgreater{}
    there is matrix \iMbox{R} s.t. \iMbox{RA = A'}
  \item
    \textbf{ECOs} transform
    \iMbox{\ds A \rightsquigarrow_{\mathrm{ECOs}} A'} =\textgreater{}
    there is matrix \iMbox{C} s.t. \iMbox{AC = A'}
  \item
    Both transform
    \iMbox{\ds A \rightsquigarrow_{\mathrm{EROs+ECOs}} A'}
    =\textgreater{} there are matrices \iMbox{R,C} s.t. \iMbox{RAC = A'}
  \end{itemize}
\item
...
\item
  ...
  \item
    ...
    to get \iMbox{AC = R^{-1}A'} =\textgreater{} useful for LU
    factorization
  \end{itemize}
\end{itemize}
...
\item
  ...
  \iMbox{\ds P(\lambda) = \lvert A - \lambda I \rvert = \sum_{i=0}^{n} a_{i}\lambda^{i}}
  ...
  \item
    ...
    \iMbox{P(\lambda)}
  \item
    \iMbox{1 \leq \text{geometric multiplicity of} \ \lambda \leq \text{algebraic multiplicity of} \ \lambda}
  \end{itemize}
\item
  ...
  \item
    \iMbox{\ds \mathrm{tr}(A) = \sum_{i} \lambda_{i}} and
    \iMbox{\ds \mathrm{\det}(A) = \prod_{i} \lambda_{ij}}
  \item
...
\item
  ...
  =\textgreater{} if \iMbox{P^{-1}} exists then
...
\item
  ...
  \item
    If \iMbox{\ds \mathbf{x}_{i},\mathbf{x}_{j}} associated to different
    eigenvalues then \iMbox{\ds \mathbf{x}_{i} \perp \mathbf{x}_{j}}
  \item
  ...
  \item
    ...
    an ONB of \iMbox{\mathbb{R}^{n}} =\textgreater{}
  ...
  \item
    ...
    of \iMbox{A} =\textgreater{} \iMbox{A = \mathbf{Q}D\mathbf{Q}^{T}}
