...
\begin{itemize}
      ...
      \item
            ...
            be \textbf{\emph{projector}} onto \underline{hyperplane} \iMbox{(\mathbb{R}\mathbf{q}_{j})^{\perp}},
            i.e. \underline{orthogonal compliment} of line \iMbox{\mathbb{R}\mathbf{q}_{j}}
            ...
            \begin{itemize}
                  ...
                  \item
                        ...
                        are iteratively applied to \iMbox{\ds\mathbf{a}_{j+1}}, removing its \underline{components along} \iMbox{\mathbf{q}_{1}},
                        then \underline{along} \iMbox{\mathbf{q}_{2}}, \emph{and so on\ldots{}}
            \end{itemize}
      \item
            Let \iMbox{\mathbf{u}^{(j)}_{k} = \left(\prod_{i=1}^{j}\mathrm{P}_{\perp \ds\mathbf{q}_{i}} \right) \mathbf{a}_{k}},
            i.e. \iMbox{\ds\mathbf{a}_{k}} \textbf{\emph{without}} its \underline{components along} \iMbox{\mathbf{q}_{1},\dots,\mathbf{q}_{j}}
      ...
      \item
            ...
            \begin{itemize}
                  ...
                  \item
                        Where \iMbox{A \in \mathbb{R}^{m \times n}} is \underline{full-rank},
...
\begin{itemize}
      \item \textbf{When clear} write \textbf{\iMbox{i}-th component} of input as
            \iMbox{i} instead of \iMbox{\mathbf{x}_{i}}

      \item \textbf{Level curve} w.r.t. to \iMbox{c \in \mathbb{R}} is all points
            s.t. \iMbox{f(\mathbf{x}) = c}

      \item
            Projecting \underline{level curves} onto \iMbox{\mathbb{R}^{n}} gives \iMbox{f}'s \textbf{contour-map}
\end{itemize}
...
\begin{itemize}
      ...
      \item \iMbox{\nabla f = \left[ \partial_{1} f, \dots, \partial_{n} f \right]^{T}}
            is \underline{gradient} of \iMbox{f} => \iMbox{(\nabla f)_{i} = \frac{\partial f}{\partial \mathbf{x}_{i}}}

      \item \iMbox{\nabla^{T} f = (\nabla f)^{T}} is \underline{transpose} of \iMbox{\nabla f}, i.e. \iMbox{\nabla^{T}f} is \underline{row vector}
\end{itemize}
...
\begin{itemize}
      ...
      \item
            It is \underline{rate-of-change} in direction \iMbox{\mathbf{u}}, where \iMbox{\mathbf{u} \in \mathbb{R}^{n}} is \underline{unit-vector}
      \item
      ...
      \item
            i.e. when \iMbox{\mathbf{x},\mathbf{u}} are \underline{parallel} => hence \iMbox{\nabla f(\mathbf{x})} is
...
\begin{itemize}
      \item
            ...
            its \iMbox{f'(x) = 0}
      \item
            \iMbox{\nabla^2 f(\mathbf{x})} is \underline{positive-definite},
...
\begin{itemize}
      ...
      \item
            ...
            \iMbox{x \in X}, shortened to \textbf{\emph{just}} \underline{``problem''}
...
\begin{itemize}
      ...
      \item
            \textbf{Well-conditioned} if \underline{all} \textbf{small} \iMbox{\delta x}
            lead to \textbf{small} \iMbox{\delta f}, i.e. if \iMbox{\kappa} is
            \textbf{small} \emph{(e.g. \iMbox{1}, \iMbox{10}, \iMbox{10^{2}})}
      \item
            \textbf{Ill-conditioned} if \underline{some} \textbf{small} \iMbox{\delta x}
...
\begin{itemize}
      ...
      \item
            If \underline{Jacobian} \iMbox{\mathbf{J}_{f}(x)} exists then \iMbox{\hat{\kappa} = \lVert \mathbf{J}_{f}(x) \rVert},
            where \underline{matrix norm} \iMbox{\lVert - \rVert} induced by \underline{norms} on \iMbox{X} and \iMbox{Y}
\end{itemize}
...
\begin{itemize}
      ...
      \item
            If \underline{Jacobian} \iMbox{\mathbf{J}_{f}(x)} exists then
...
\begin{itemize}
      \item
            \iMbox{\mathbf{A} \in \mathbb{C}^{m \times m}} is \underline{well-conditioned} if
            \iMbox{\kappa(\mathbf{A})} is \textbf{small}, \underline{ill-conditioned} if
            \textbf{large}
      \item
...
Given a \underline{problem} \iMbox{f: X \to Y}, an \textbf{algorithm} for
\iMbox{f} is \iMbox{\tilde{f}: X  \to Y}

\begin{itemize}
      \item
            Input \iMbox{x \in X} is first \underline{rounded} to \iMbox{\mathrm{fl}(x)},
...
\begin{itemize}
      ...
      \item
            ...
            \underline{condition number} \iMbox{\kappa(x)} then \underline{relative error}
            \iMbox{\frac{\|\tilde{f}(x)-f(x)\|}{\|f(x)\|} = O\left(\kappa(x)\epsilon_{\mathrm{mach}}\right)}
\end{itemize}

\underline{Accuracy, stability, backwards stability} are \textbf{norm-independent} for fin-dim \iMbox{X,Y}

\subsection*{Big-O meaning for numerical analysis}

In \underline{complexity analysis} \iMbox{f(n)=O(g(n))} as \iMbox{n \to \infty}

But in \underline{numerical analysis} \iMbox{f(\epsilon)=O(g(\epsilon))} as \iMbox{\epsilon \to 0},
...
\begin{itemize}
      ...
      \item
            \iMbox{O(g)} is \underline{set of functions}
            \iMbox{\{ f \ \ : \ \ \limsup_{\epsilon \to 0} \left. {\lVert f(\epsilon) \rVert} \ \middle/ \ {\lVert g(\epsilon) \rVert} \right. < \infty \}}
\end{itemize}

\hSep % ---

\textbf{Smallness} \underline{partial order} \iMbox{\ds O(g_{1}) \preceq O(g_{2})}
defined by \underline{set-inclusion} \iMbox{\ds O(g_{1}) \subseteq O(g_{2})}

\begin{itemize}

      \item
            i.e. as \iMbox{\epsilon \to 0}, \iMbox{g_{1}(\epsilon)} \underline{goes to zero}
      ...
      \item
            ...
            \begin{itemize}
                  \item e.g. \iMbox{\dots, O(\epsilon^3) \prec O(\epsilon^2) \prec O(\epsilon) \prec O(1)}
            \end{itemize}
      \item
            \textbf{Maximum}:
            ...
            \begin{itemize}
                  \item e.g. \iMbox{O(\max(\epsilon^{k}, \epsilon))=O(\epsilon)}
            \end{itemize}
\end{itemize}

\hSep % ---

Using \underline{functions} \iMbox{f_{1},\dots,f_{n}}, let \iMbox{\mathbfit{\Phi}(f_{1},\dots,f_{n})}
be \underline{formula} \textbf{defining some function}

\begin{itemize}

      \item
            Then \iMbox{\mathbfit{\Phi}(O(g_{1}),\dots,O(g_{n}))} is the \underline{class of functions}
      ...
      \item
            ...
            \end{itemize}
      \item
            \underline{Special case}: \iMbox{f = \mathbfit{\Phi}(O(g_{1}),\dots,O(g_{n}))}
...
\begin{itemize}
      ...
      \item
            ...
            \iMbox{\beta^{t-1} \leq m \leq \beta^t-1} for \underline{unique} \iMbox{m} and
            \iMbox{e}
      \item
            \iMbox{\mathbf{F}\subset \mathbb{R}} is \underline{idealized} 
...
\begin{itemize}
      \item
            \underline{Half the gap} between \iMbox{1} and \underline{next largest FP}
      \item
...
\begin{itemize}
      ...
      \item
            ...
            \begin{itemize}
                  \item
                        \underline{Caveat:} \iMbox{\epsilon_{\mathrm{mach}} = \frac{1}{2}\beta^{1-t}}
                        must be \underline{scaled} by factors \underline{on the order of} \iMbox{2^{3/2},2^{5/2}}
      ...
      \item
            ...
            \begin{itemize}
                  ...
                  \item
                        ...
                        where \iMbox{\lvert x \rvert_{i} = \lvert x_{i} \rvert} is \underline{vector}
                        and \iMbox{\phi(n)} is \underline{small function} of \iMbox{n}
            \end{itemize}
      \item
            \textbf{Summing a series} is \underline{more stable} if terms \underline{added in order of increasing magnitude}
\end{itemize}

For \textbf{FP matrices}, let
\iMbox{\lvert M \rvert_{ij} = \lvert M_{ij} \rvert}, i.e. matrix
\iMbox{\lvert M \rvert} of \underline{absolute values} of \iMbox{M}
\begin{itemize}
